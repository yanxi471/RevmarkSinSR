import os, sys, math, time, random, datetime, functools, shutil
import lpips
import numpy as np
...
def _resolve_stage_snapshot_iters(configs):
    """训练 yaml 中 stage_snapshot_iters；未设则用 milestones。"""
    raw = configs.train.get('stage_snapshot_iters', None)
    if raw is None or (isinstance(raw, str) and raw.strip() in ('~', '', 'null')):
        raw = configs.train.get('milestones', []) or []
...
class TrainerBase:
    ...
    def build_iqa(self):
        ...
        try:
            ...
            if hasattr(self, "logger") and self.logger is not None:
                self.logger.warning(f"pyiqa 不可用（{e}），验证阶段将不记录 CLIP-IQA / MUSIQ。")
    ...
    def train(self):
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            # save checkpoint 先于 validation：验证若 OOM/异常退出时已落盘，避免丢 milestone 权重
            save_freq = int(self.configs.train.save_freq)
            ms = _resolve_stage_snapshot_iters(self.configs)
            need_save = (ii + 1) % save_freq == 0
            if ms and (ii + 1) in ms and (ii + 1) % save_freq != 0:
                need_save = True
            if need_save:
                self.save_ckpt()
    ...
    def _maybe_mirror_stage_snapshot(self):
        """
        将当前 iter 在 ckpt_dir / ema_ckpt_dir 下的 .pth 复制到 stage_snapshot_dir/iter_XXXXXX/，
        与 save_dir 下日志目录双份保存，防止误删 saved_logs 时丢失阶段权重。
        """
        if self.rank != 0:
            return
        rel = self.configs.train.get('stage_snapshot_dir', None)
        if rel is None or (isinstance(rel, str) and str(rel).strip() in ('', '~', 'null')):
            return
        if not bool(self.configs.train.get('stage_snapshot_every_save', False)):
            targets = set(_resolve_stage_snapshot_iters(self.configs))
            if not targets:
                return
            if self.current_iters not in targets:
                return
        root = Path(rel)
        if not root.is_absolute():
            root = Path.cwd() / root
        stage_dir = root / f'iter_{self.current_iters:06d}'
        stage_dir.mkdir(parents=True, exist_ok=True)
        n = 0
        for d in (self.ckpt_dir, self.ema_ckpt_dir):
            if not d.exists():
                continue
            for f in sorted(d.glob(f'*_{self.current_iters}.pth')):
                shutil.copy2(f, stage_dir / f.name)
                n += 1
        if hasattr(self, 'logger') and self.logger is not None:
            self.logger.info(f'[stage_snapshot] copied {n} file(s) -> {stage_dir}')
        ...
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                with torch.no_grad():
                    ...
                    if getattr(self, "_pyiqa_available", False):
            ...
            if getattr(self, "_pyiqa_available", False):
                ...
                self.logger.info(
                    f"Validation Metric: MUSIQ={mean_musiq:5.2f}, clipiqa={mean_clipiqa:6.4f}..."
                )
            if "gt" in data:
...
class TrainerDistillDifIR(TrainerDifIR):
    ...
    def log_step_train(self, loss, tt, batch, z_t, z0_pred, flag=False, phase='train'):
        ...
        if self.rank == 0:
            ...
            num_timesteps = self.base_diffusion.num_timesteps
            record_steps = [1, num_timesteps //2, num_timesteps]
            lf0 = max(1, int(self.configs.train.log_freq[0]))
            log_every_train_line = lf0 == 1
            if log_every_train_line or self.current_iters % lf0 == 1:
                self.loss_mean = {key:torch.zeros(size=(len(record_steps),), dtype=torch.float64)
                                  for key in loss.keys()}
                self.loss_count = torch.zeros(size=(len(record_steps),), dtype=torch.float64)
            for jj in range(len(record_steps)):
            ...
            if (log_every_train_line or self.current_iters % lf0 == 0 or self.current_iters == 1) and flag:
                if torch.any(self.loss_count == 0):
                    self.loss_count += 1e-4
                for key in loss.keys():
                    self.loss_mean[key] /= self.loss_count
